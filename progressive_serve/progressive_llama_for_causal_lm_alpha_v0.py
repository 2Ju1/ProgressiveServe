...
class ProgressiveLlamaForCausalLMAlpha(nn.Module):
    ...
    def _load_prune_log(self, model_path: str) -> Optional[dict]:
        ...
        try:
            ...
            if 'split' not in prune_log:
                print(f"'split' field not found in prune_log.json")
                return None
            
            if 'B' not in prune_log['split'] or 'C' not in prune_log['split']:
                print(f"'B' or 'C' not found in split")
                return None
    ...
    def load_weights(self, weights: Iterable[Tuple[str, torch.Tensor]]):
        ...
        if gate_up_keys:
            ...
            print("   NO gate_proj or up_proj in Stage 1 checkpoint!")
        ...
        if missing_keys:
            print(f"\n Found {len(missing_keys)} missing weights")
            ...
            for layer_idx in sorted(missing_by_layer.keys()):
                ...
                if layer_idx in self.inactive_layer_indices:
                    ...
                    print(f"    Layer {layer_idx}: Missing {len(layer_keys)} weights (ACTIVE layer!)")
    ...
    def advance_to_stage2(
        ...
        if self.prune_info is None:
            print(" Warning: No prune_log available. Using fallback.")
